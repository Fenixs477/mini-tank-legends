...
export class BotBrain {
  ...
  decide(game: any): BotDecision {
    ...
    if (target) {
      ...
      if (game.world.collides(probeX, probeZ, 2)) { turn += 0.8; throttle = 0.5; }
      this.fireCooldown -= game.dt;
      const turDiff = Math.abs(this._angleDiff(turretAngle, me.turretAngle));
      if (turDiff < 0.15 && dist < me.def.shellRange && me.reloadLeft <= 0) fire = true;
    } else {
