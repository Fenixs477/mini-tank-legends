/* world.ts — battlefield generation with biomes, lakes, walls, camo */
import * as THREE from 'three';
import { CONFIG, COLORS } from './config';
import { generateNoiseTexture, generateDistortionTexture } from './shaders';
import { Models } from './models';
import type { WallData, LakeData, WavePlaneData } from './types';

export class World {
  scene: THREE.Scene;
  walls: WallData[] = [];
  trees: THREE.Object3D[] = [];
  bushes: { x: number; z: number; mesh?: THREE.Object3D; r?: number }[] = [];
  lakes: LakeData[] = [];
  size: number;
  half: number;
  treesPlaced = true;
  groundMat: THREE.MeshStandardMaterial | null = null;
  ground: THREE.Mesh | null = null;
  sunLight: THREE.DirectionalLight | null = null;
  _noiseTex: THREE.CanvasTexture | null = null;
  _distTex: THREE.CanvasTexture | null = null;
  _foamRings: { mesh: THREE.Mesh; phase: number }[] = [];
  _wavePlanes: THREE.Mesh[] = [];
  _customMeshes: THREE.Object3D[] = [];
  ...
  _makeGround(): void {
    ...
    for (let p = 0; p < 4; p++) {
      g.beginPath(); let x = Math.random() * 1024, y = Math.random() * 1024;
      g.moveTo(x, y);
  ...
  _makeLakes(): void {
    ...
    for (const l of lakeData) {
      ...
      for (let i = 0; i < emitterCount; i++) {
        ...
        const dd = 0.3 + Math.random() * 0.2;
        const fg2 = new THREE.PlaneGeometry(w, dd);
        const fm2 = new THREE.MeshBasicMaterial({
          color: 0xffffff, transparent: true, opacity: 0,
          depthWrite: false, blending: THREE.AdditiveBlending, side: THREE.DoubleSide,
        });
        const mesh = new THREE.Mesh(fg2, fm2);
        ...
        mesh.userData = { angle, radius: startR, speed: 1.2 + Math.random() * 0.8, spawnR: startR, lake: l, delay: Math.random() * 4 };
        this.scene.add(mesh);
  ...
  _lakeRad(l: LakeData, a: number): number {
  ...
  update(dt: number, time: number): void {
    for (const wp of this._wavePlanes) {
      const ud = wp.userData as WavePlaneData;
      const lake = ud.lake;
      const shoreR = this._lakeRad(lake, ud.angle);
      const spawnR = Math.max(2, shoreR - 10);
      const targetR = shoreR - 0.5;
      if (ud.delay > 0) {
        ud.delay -= dt;
        if (ud.delay <= 0) { ud.delay = -1; ud.radius = spawnR; }
        continue;
      }
      ...
      (wp.material as THREE.MeshBasicMaterial).opacity = Math.sin(t * Math.PI) * 0.55 + 0.05;
      const s = 1.0 + t * 0.4;
  ...
  hidingIn(x: number, z: number): string | null {
    for (const b of this.bushes) {
      if (Math.hypot(x - b.x, z - b.z) < (b.r || CONFIG.BUSH_HIDE_RADIUS)) return 'bush';
    }
    for (const t of this.trees) {
      if (Math.hypot(x - (t as any).x, z - (t as any).z) < CONFIG.TREE_HIDE_RADIUS) return 'tree';
    }
    return null;
  }

  camoFactor(x: number, z: number): number {
  ...
  _inLake(x: number, z: number, pad = 0): boolean {
    for (const l of this.lakes) {
      const dx = x - l.x, dz = z - l.z;
      const dist = Math.hypot(dx, dz);
      const a = Math.atan2(dz, dx);
      if (dist < this._lakeRad(l, -a) + pad) return true;
    }
    return false;
  }

  lakeAt(x: number, z: number): LakeData | null {
    for (const l of this.lakes) {
      const dx = x - l.x, dz = z - l.z;
      const dist = Math.hypot(dx, dz);
      const a = Math.atan2(dz, dx);
  ...
  collidesWallsOnly(x: number, z: number, r: number): boolean {
  ...
  loadCustomMapData(data: any): void {
    ...
    data.objects.forEach((d: any) => {
      ...
      if (d.isModel && d.modelName) {
        ...
        const loadModel = (grp: THREE.Group | null) => {
          if (!grp) return;
          const clone = grp.clone(true);
          clone.position.set(d.x, d.y, d.z);
          clone.scale.set(d.sx, d.sy, d.sz);
          clone.rotation.y = d.ry;
          clone.traverse(o => { if ((o as THREE.Mesh).isMesh) { (o as THREE.Mesh).castShadow = true; (o as THREE.Mesh).receiveShadow = true; } });
          this.scene.add(clone);
          this.scene.remove(mesh);
          if (d.type === 'water') {
            const idx = this.lakes.findIndex((l: any) => l.x === d.x && l.z === d.z);
            if (idx >= 0) this.lakes[idx].mesh = clone;
          } else if (d.type === 'bush') {
            const idx = this.bushes.findIndex((b: any) => b.mesh === mesh);
            if (idx >= 0) this.bushes[idx].mesh = clone;
          } else {
            const idx = this.walls.findIndex((w: any) => w.mesh === mesh);
      ...
      const mat = (d.type === 'water')
        ? new THREE.MeshBasicMaterial({ color: 0x61B2FF, transparent: true, opacity: 0.85 })
  ...
  renderToCanvas(ctx: CanvasRenderingContext2D, w: number, h: number, _opts?: any): void {
  ...
  worldToMap(x: number, z: number, canvasSize: number): [number, number] {
